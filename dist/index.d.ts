...
export declare function useForm<F extends AnyFormValues, S extends readonly StringKey<F>[] | undefined = undefined>(form: Form<F>, signals?: S): {
	models: {
		[K in S extends readonly any[] ? S[number] : StringKey<F>]: Ref<F[K], F[K]>;
	};
	fields: {
		[K_1 in S extends readonly any[] ? S[number] : StringKey<F>]: any;
	};
	submit: () => void;
};
export declare function makeInputBinder<F extends AnyFormValues, K extends keyof F & string>(form: FormLike<F>, name: K): {
	modelValue: F[K];
	"onUpdate:modelValue": (v: F[K]) => void;
	name: K;
	fields: {
		[P in keyof F]: FieldShape<F[P]>;
	};
};
